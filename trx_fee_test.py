...
def find_source_dest_pair(prev, n, nxt):
    sources=[]
    dest=[]
    # Source identification
    paths = nx.shortest_path(G, target=nxt)
    for i in paths.values():
        if [prev, n, nxt] in [i[j:j+3] for j in range(len(i)-2)]:
            sources.append(i[0])

    # Destination finding
    paths = nx.single_source_dijkstra_path(G, prev)
    for i in paths.values():
        if [prev, n, nxt] in [i[j:j+3] for j in range(len(i)-2)]:
            dest.append(i[-1])
    return sources,dest
...
def trx_amt_test(trx_amt=1000):
    ...
    for d in dests:
        bf=[]
        pf=[]
        path=nx.shortest_path(G,source=att,target=d)
        trx=actual_amt
        # print(path, trx)
        for i in range(len(path) - 1, 0, -1):
            v, u = path[i], path[i - 1] 
